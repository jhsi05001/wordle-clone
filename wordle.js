...
function compareWord() {
    ...
    for (let i = 0; i < word.length; i++) {
        ...
        if (pos === -1) {   
            ...
            colorKeyboardKey(guess.charAt(i), "grey");
        } else {
            if (guess.charAt(i)=== word.charAt(i)) {
                ...
                colorKeyboardKey(guess.charAt(i), "green");
            } else {
                // change to yellow
                ...
                colorKeyboardKey(guess.charAt(i), "yellow");
            } 
...
function colorKeyboardKey(key, color) {
    const elements = document.getElementsByClassName("key");

    for (const element of elements) {
        if (element.getAttribute("data-cell-index") === key.toUpperCase()) {
